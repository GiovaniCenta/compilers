...
TAC *generate_code(AST *node)
{
    ...
    case AST_IF_ELSE:
        ...
        break;
    case AST_UNTIL:
        if (can_optimize_until(node) == 1)
        {
            
            result = create_tac_until_optimized(node);
...
TAC *create_tac_until_optimized(AST *node)
{
    char *symbolAux, *fimAux;
    HASH_NODE *auxTemp = 0;
    TAC *auxMove, *auxAdd = 0;
    int i = 0;
    ...
    if (node->son[0])
    {
        if (node->son[0]->son[0])
        {
            symbolAux = node->son[0]->son[0]->symbol->text;
        }
        if (node->son[0]->son[1])
        {
            fimAux = node->son[0]->son[1]->symbol->text;
        }
    }
    ...
    if (node->son[1])
    {
        if (node->son[1]->son[0])
        {
            if (node->son[1]->son[0]->son[0])
            {
                if (node->son[1]->son[0]->son[0]->son[1])
                {
                    if (node->son[1]->son[0]->son[0]->son[1]->type == AST_ADD)
                    {
                        if (node->son[1]->son[0]->son[0]->son[1]->son[0])
                        {
                            if (node->son[1]->son[0]->son[0]->son[1]->son[0]->symbol->text == symbolAux)
                            {

                                while (i <= atoi(node->son[0]->son[1]->symbol->text))
                                {
                                    i = i + atoi(node->son[1]->son[0]->son[0]->son[1]->son[1]->symbol->text); //dava pra mudar aqui depois

                                    auxTemp = make_temp();
                                    auxMove = tac_create(TAC_ADD, auxTemp, node->son[1]->son[0]->son[0]->son[1]->son[0]->symbol, node->son[1]->son[0]->son[0]->son[1]->son[1]->symbol);

                                    auxAdd = tac_join(tac_join(auxMove, tac_create(TAC_MOVE, node->son[1]->son[0]->son[0]->son[1]->son[0]->symbol, auxTemp, 0)), auxAdd);
                                }
                            }
                        }
                    }
...
int can_optimize_until(AST *node)
{
    if (node->son[1])
    {
        if (node->son[1]->son[0])
