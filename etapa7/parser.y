...
cmd_list:
    ...
    |cmd error                    { $$ = 0; syntax_error("missing ;");  }
...
flow_control:
    ...
    |KW_IF '(' error ')' cmd      { $$ = 0; syntax_error("missing logical expression"); }
    |KW_IF '(' expression ')' error     { $$ = 0; syntax_error("missing command after if expression"); }
    |KW_IF '(' expression ')' cmd KW_ELSE cmd  { $$ = astCreate(AST_IF_ELSE,0,$3,$5,$7,0); }
    |KW_IF '(' error ')' cmd KW_ELSE cmd  { $$ = 0; syntax_error("missing logical expression"); }
    |KW_UNTIL '(' expression ')' cmd       { $$ = astCreate(AST_UNTIL,0,$3,$5,0,0); }
    |KW_UNTIL '(' error ')' cmd      { $$ = 0; syntax_error("missing logical expression"); }
    ;

block: '{' cmd_list '}' { $$ = astCreate(AST_BLOCK,0,$2,0,0,0); }
|'{' cmd_list error { $$ = 0; syntax_error("missing }"); }
...
expression_list:
    ...
    | expression error expression_list     { $$ = 0; syntax_error("missing ,"); }
    ;
...
parameterslist: ',' parameter parameterslist                 { $$ = astCreate(AST_PARAMETER_LIST,0,$2,$3,0,0); }
    | error parameter parameterslist     { $$ = 0; syntax_error("missing ,"); }
...
function:
    ...
    | error ':' identifier '(' parameter_init parameterslist_init ')' block     { $$ = 0; syntax_error("missing type"); }
    | type error identifier '(' parameter_init parameterslist_init ')' block     { $$ = 0; syntax_error("missing : "); }
